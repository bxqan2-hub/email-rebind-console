...
class AppTests(unittest.TestCase):
    ...
    def test_page_explains_automatic_rotation_and_failed_pool_controls(self):
        ...
        self.assertIn("data-success-account-id", script)
        self.assertIn("renderSuccessTrialInfo", script)
        self.assertIn("trial_zero_trial_eligible", script)
        self.assertIn("/api/detection-proxies/import", script)
        self.assertIn("selectedSuccessAccounts", script)
        self.assertIn("copySelectedAccessTokens", script)
        self.assertIn("pushSelectedAccessTokensToGCash", script)
        self.assertIn("cleanupSelectedAccounts", script)
        self.assertIn("selectedResultTasks", script)
        self.assertIn("cleanupSelectedResults", script)
        self.assertIn('id="selectSuccessAll"', html)
        ...
        self.assertEqual(html.count('data-export-copy-format'), 2)
        self.assertIn('value="full">包含原邮箱（默认）', html)
        self.assertIn('value="without_old_email">不含原邮箱', html)
        self.assertIn("换绑后邮箱----密码----2FA----AT", html)
        self.assertIn("email-rebind:export-copy-format", script)
        ...
        self.assertIn('id="deleteAllDetectionProxies"', html)
        self.assertIn('/api/detection-proxies', script)
        self.assertIn('deleteAllDetectionProxies', script)
        self.assertIn('id="detectionProxyBody"', html)
        self.assertIn('detection-proxy-table', html)
        self.assertNotIn('data-delete-detection-proxy', script)
        self.assertIn('proxy-pool-table', html)
        self.assertIn('id="pushSelectedGCash"', html)
        self.assertIn('id="cleanupSelectedAccounts"', html)
        self.assertIn('id="resultVisibleRows"', html)
        self.assertIn('class="task-list result-task-list"', html)
        self.assertIn("function resultVisibleLimit", script)
        self.assertIn("email-rebind:result-visible-rows", script)
        self.assertIn("const rows=STATE.tasks", script)
        self.assertIn("applyResultVisibleLimit", script)
        self.assertIn("scrollTop", script)
        self.assertIn('id="resultScrollHint"', html)
        self.assertIn('id="selectResultAll"', html)
        self.assertIn('id="cleanupSelectedResults"', html)
        self.assertIn("clearFinishedTasks", script)
        self.assertIn("$('#copySuccessResultsBottom').onclick", script)
        self.assertIn("密码/2FA/AT", script)
        ...
        self.assertIn(">复制AT</button>", html)
        self.assertIn(">推送GCash</button>", html)
        self.assertIn("retrySettingInitialized", script)
        self.assertNotIn("document.activeElement!==retryInput", script)
        self.assertNotIn("id=\"downloadExport\"", html)
        self.assertIn("纯协议模式", html)
        self.assertIn('id="startBrowserRebind"', html)
        self.assertIn('id="pairVisibleRows"', html)
        self.assertIn('pairing-table', html)
        self.assertIn("rebind_mode:'browser'", script)
        self.assertIn("open_roxy_after", script)
        self.assertIn('class="table-wrap success-table-wrap"', html)
        self.assertIn(".pairing-table,.success-table-wrap", script)
        self.assertIn('class="success-table"', html)
        self.assertIn("CDP端口", html)
        self.assertNotIn("<th>Profile</th>", html)
        self.assertIn("function shortTime", script)
        self.assertIn("email-stack", script)
        self.assertIn("applyPairVisibleLimit", script)
        self.assertIn("email-rebind:pair-visible-rows", script)
        self.assertIn("roxy_cdp_port", script)
        self.assertIn('>GCash</button>', script)
        styles = Path("static/app.css").read_text(encoding="utf-8")
        self.assertIn(".success-table th:not(:first-child){width:16%}", styles)
        self.assertIn("grid-template-columns:repeat(3,minmax(0,1fr))", styles)
        self.assertIn("vertical-align:middle;text-align:center", styles)
        self.assertIn(".success-table .btn{width:100%", styles)
        ...
        self.assertIn("data-replacement-id", script)
        self.assertIn("selectedReplacements", script)
        self.assertIn("deleteSelectedReplacements", script)
        self.assertIn('id="selectReplacementAll"', html)
        self.assertIn('id="selectReplacementAllTable"', html)
        self.assertIn('id="deleteSelectedReplacements"', html)

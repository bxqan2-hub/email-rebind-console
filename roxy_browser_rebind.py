# -*- coding: utf-8 -*-
"""65114c4 full Roxy-browser email rebind flow, kept as an explicit opt-in mode."""
...
def perform_email_rebind(
    *,
    ...
    try:
        ...
        source_email_api_url = "" if password_totp_login else str(source_api_url or "").strip()
        previous_old_login_otp = None
        if source_email_api_url:
            try:
                previous_old_login_otp = mail_api.read_current_otp(source_email_api_url, old_email, timeout=4)
            except Exception:
                pass
        ...
        old_session = _complete_login(
            ...
            email_api_url=source_email_api_url, previous_email_otp=previous_old_login_otp,
            auth_method=auth_method,
            email_label="原邮箱",
            stop_check=stop_check,
        )

...
- “复制格式”：默认“包含原邮箱”，密码账号输出 `原邮箱----换绑后邮箱----密码----2FA----AT`；选择“不含原邮箱”后输出 `换绑后邮箱----密码----2FA----AT`。URL 账号同样只去掉原邮箱列，其余字段保持原样。两处格式选择同步并在当前浏览器记住选择，批量复制和单个账号的“结果”按钮均按所选格式复制；“复制AT”不受影响。
...
| 验证码已提交但没有收到明确结果 | 标记 `review` | 记录可能的新邮箱并标记 `review` | 可只登录新邮箱核验，或删除本地账号记录 |
| 服务端已确认换绑，但新邮箱首次登录 / AT 获取失败 | 标记 `used` | 保留新身份及登录资料 | 自动重新协议登录一次；仍失败时标记 `pending_at`，可手动重登 |
| 验证码被明确拒绝（Invalid OTP） | 释放回 `available` | 保留原邮箱，标记失败 | 修正验证码问题后再重试，不标记已换绑 |
...
服务端确认邮箱变更时立即保存新邮箱、密码/2FA 和新邮箱取码 URL；
AT 尚未取得不会撤销换绑成功状态。首次新邮箱登录失败后自动重新建立一次
纯协议登录，补登成功直接保存 AT。补登仍失败显示“已换绑 · 待 AT”。

“重新登录获取 AT”使用账号保存的资料，不依赖任务记录或替换邮箱池。
密码 + 2FA 账号直接使用纯协议；仅邮箱 API 的账号使用保留的取码 URL 登录。
删除号池记录前自动保存所需 URL，清理任务日志不影响补登。
待 AT 和待核验账号均允许删除本地记录，正在执行的任务仍须先结束。

启动时按历史 trace 中匹配的原/新邮箱和实际 verify 响应修复旧版 review：
确认成功的转为 pending_at，明确 Invalid OTP 拒绝的保留原身份，提交结果未知的
继续保留 review。迁移不会自动发起新的换绑请求。
